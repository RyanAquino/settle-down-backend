...
Constructed fresh per request. `__init__` signs in to Firebase via `pyrebase` and caches the token in Redis (~3500s ≈ 58 min). `get_groups` / `get_group_members_by_group` / `get_group` hit the Settle Up REST API and cache results for ~24h (`timeout=86500`) — **so changes made in Settle Up won't appear until that cache expires.** For `get_group` this now includes the group's currency, so a currency changed in Settle Up won't be reflected in transactions filed through this API for up to 24h.
...
`backend_api/tests/conftest.py` provides the `settle_up_client` fixture: a real `SettleUpClient` with `pyrebase`, `requests`, and `cache` all patched, so no login/network/Redis happens. The mocked `requests.get` is routed by URL: group-document URLs (`/groups/...`, not `/userGroups/...`) return the mutable `group_json` dict on the fixture namespace (default currency JPY), and every other URL falls through to `return_value` — that's what the members-retargeting idiom below uses. The mocked members endpoint returns exactly **Member 1 and Member 2** (`GROUP_MEMBERS`), and the split-related assertions in `test_transaction.py` depend on that two-member group; `test_four_member_scenarios.py` retargets the same mock to a four-member group to cover partial participation and duplicate-item splits. Tests exercise `_compute_transaction()` directly across tax-included / tax-excluded / shared-split / fractional-tax scenarios. pytest is configured in `pyproject.toml` (`DJANGO_SETTINGS_MODULE=settledown.settings`, `test_*.py`).
